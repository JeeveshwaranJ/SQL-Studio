...
// Dynamically import DesignerLayout to disable server-side rendering for browser-only canvas components
const DesignerLayout = dynamic(() => import("../../components/layout/DesignerLayout"), {
  ssr: false,
  loading: () => (
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-[#1e1e1e] text-[#d4d4d4]">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-2.5 text-xs text-zinc-400">
          ...
          <span>Starting Schema Designer Workspace...</span>
        </div>
      </div>
    </div>
  ),
...
export default function DesignerPage() {
  ...
  // Initialize the database on mount if the user lands directly on the designer
  useEffect(() => {
    if (!adapter) {
      initDb();
    }
