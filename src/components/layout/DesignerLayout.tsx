...
export default function DesignerLayout() {
  ...
  const { syncSchema, selectedTableName } = useDesignerStore();
  ...
  // Sync schema in designer when db or route changes
  useEffect(() => {
    const adapter = useDbStore.getState().adapter;
    if (adapter) {
      syncSchema(adapter as any, dbName);
    }
  }, [dbName, syncSchema]);

  // Programmatically expand the right side panel when a table or column is selected
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col flex-1 h-screen w-full select-none overflow-hidden bg-background text-foreground animate-fadeIn text-xs">
      ...
      <header className="flex h-14 items-center justify-between border-b border-border bg-card px-4 shrink-0 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground leading-none">Interactive Schema Designer</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={initDb}
              className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded border border-border bg-background hover:bg-accent cursor-pointer transition-all animate-pulse"
      ...
      <main className="flex-grow flex min-h-0 min-w-0 bg-background text-foreground transition-colors relative overflow-hidden">
        ...
        {/* Center Panel: ER Flow Canvas */}
        <div className="flex-grow flex flex-col min-h-0 min-w-0 relative bg-background">
          <DesignerCanvas />
        </div>

        {/* Right panel: Collapsible properties Sidebar */}
        <div 
          className={`flex flex-col bg-card min-h-0 shrink-0 border-l border-border transition-all duration-200 ${
            isRightCollapsed ? "w-0 opacity-0 overflow-hidden" : "w-80 opacity-100"
          }`}
        >
          {!isRightCollapsed && (
            <div className="flex flex-col h-full relative">
              ...
              <button 
                ...
                className="absolute top-2 right-2 p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground cursor-pointer z-10 transition-colors"
